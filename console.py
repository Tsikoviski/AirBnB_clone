...
"""
The console v: 0.0.1
Contain entry point of cmd interpreter
"""
...
class HBNBCommand(cmd.Cmd):
    ...
    def my_errors(self, line, num_of_args):
        """Display err msgs to usr

        Args:
            line(any): get usr inp using cmd line
            num_of_args(int): numb of input args

        Description:
            Display output to usr based on
             input cmds.
    ...
    def do_quit(self, line):
        ...
        Args:
            line(args): input arg for quitting
            the terminal
    ...
    def do_create(self, line):
        """Create new inst of @cls_name class,
        and print new instance's ID.

        Args:
            line(args): Argument to enter with cmd: <class name>
            Example: 'create Usr'
        ...
        """
        args[0] contain class name, create new inst
        of that class update 'updated_at' attrib,
    ...
    def do_show(self, line):
        """Print string rep of  inst.

        Args:
            line(line): enter with cmd <class name> <id>
            Example: 'show Usr 1234-1234-1234'
    ...
    def do_destroy(self, line):
        """Delete  instance of certain class.
    ...
    def do_all(self, line):
        """Show all instances, or instances of certain class
    ...
    def do_update(self, line):
        """Update inst based on class name
        and id by adding or updating attrib

        Args:
            line(args): receive commands:
            <class name> <id> <attrib name> "<attrib value>"
            Example: 'update Usr 1234-1234-1234 my_name "Bob"'
    ...
    def my_count(self, class_n):
        """
        Method count inst of certain class
        """
        count_instance = 0
    ...
    def default(self, line):
        """Method to take care of following cmds:
        <class name>.all()
        <class name>.count()
        <class name>.show(<id>)
        <class name>.destroy(<id>)
        <class name>.update(<id>, <attrib name>, <attrib val>)
        <class name>.update(<id>, <dict repres)

        Description:
            Create list reps of functional models
            Then use functional methods to implement usr
            cmds, by validating all input cmds
